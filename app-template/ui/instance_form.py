...
class RepeatingInstanceForm(ttk.Frame):
    def __init__(self, parent, templates, instance=None):
        ...
        template_id = instance.schedule_template_id if instance else (templates[0].id if templates else None)

        self.name_var = tk.StringVar(value=name)
        self.description_var = tk.StringVar(value=description)
        self.length_var = tk.StringVar(value=str(length))

        self.template_id_by_name = {t.name: t.id for t in templates}
        initial_template_name = next(
            (t.name for t in templates if t.id == template_id),
            (templates[0].name if templates else ""),
        )
        self.template_name_var = tk.StringVar(value=initial_template_name)
        ...
        ttk.Combobox(
            self, textvariable=self.template_name_var, state="readonly",
            values=[t.name for t in templates], width=36,
    ...
    def get_instance_fields(self) -> dict:
        try:
            ...
            length = 90
        template_id = self.template_id_by_name.get(self.template_name_var.get())
