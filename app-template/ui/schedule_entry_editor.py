...
Reordering goes through the shared ui/drag_reorder.py::DragReorder helper
(extracted from this module's own original drag/ghost/index-at-point
implementation once Board columns/statuses in ui/settings.py needed the
identical technique). The drag handle is a small glyph on the left of each
row rather than the whole row, so editing doesn't fight with starting a
drag.
"""
...
def build_entry_list_editor(parent, ctx, entries: list, on_change=None) -> None:
    ...
    def on_drop(start_index: int, insert_at: int) -> None:
        entry = entries.pop(start_index)
        entries.insert(insert_at, entry)
        render_entries()

    reorder = DragReorder(ctx, on_drop)
    ...
    def render_entries() -> None:
        ...
        for idx, entry in enumerate(entries):
            ...
            reorder.bind_handle(handle, card, idx, name)
